 ...
 *
 *   // network
 *   CSosNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.sos.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CSosNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
// // can be under: SosNetwork
// //   -- signature requirements : Key1(User)
// class CSosNetworkVariable : public CGovernanceObject
// {
// private:

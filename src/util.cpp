...
const char * const BITCOIN_CONF_FILENAME = "sos.conf";
const char * const BITCOIN_PID_FILENAME = "sosd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sos" is a composite category enabling all Sos-related debug output
            if(ptrCategory->count(string("sos"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SosCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SosCore
    // Mac: ~/Library/Application Support/SosCore
    // Unix: ~/.soscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SosCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SosCore";
#else
    // Unix
    return pathRet / ".soscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sos.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sos.conf
